...
const ROLE_CONFIG = {
  ...
  fund: { label: 'Fund Manager', icon: 'leaderboard', color: '#25a475' },
  financial: { label: 'Accountant', icon: 'account_balance', color: '#D97706' },
  analyst: { label: 'Analyst', icon: 'query_stats', color: '#b66dff' },
}
...
export default function Dashboard() {
  ...
  const renderDashboard = () => {
    if (!analyzeResult) return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        <span className="material-symbols-outlined text-[80px] text-slate-700">query_stats</span>
        <p className="text-slate-500 text-sm">종목코드를 입력하거나 파일을 업로드해주세요</p>
        <p className="text-slate-600 text-xs">CSV · Excel · JSON 모든 형태 지원</p>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[#13131b] text-[#e4e1ed]">
      {/* 상단 헤더 - 로고 + 직군 탭만 */}
      <header className="fixed top-0 w-full z-50 bg-slate-900/60 backdrop-blur-md border-b border-slate-800/50 shadow-2xl shadow-black/50">
        ...
        {/* 직군 탭 */}
        <div className="flex items-center px-6 border-t border-slate-800/50">
          ...
          <div className="ml-auto">
            <span className="text-[10px] text-slate-700 font-mono">Skills.md v0.1</span>
          </div>
        </div>
      </header>
      ...
      <div className="pt-[120px] flex min-h-screen">
        ...
        {/* 좌측 입력 패널 (30%) */}
        <div className="w-[20%] min-h-[calc(100vh-105px)] border-r border-slate-800 p-8 flex flex-col gap-8 bg-slate-900/30">

          {/* 현재 직군 표시 */}
          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4">
            <p className="text-[11px] font-bold text-indigo-400 uppercase tracking-widest mb-2">Current Role</p>
            <div className="flex items-center gap-3">
              <span className="material-symbols-outlined text-2xl" style={{ color: config.color }}>{config.icon}</span>
              <span className="text-lg font-bold text-white" style={{ fontFamily: 'Manrope' }}>{config.label}</span>
            </div>
          </div>

          {/* 종목코드 입력 */}
          <div>
            <p className="text-sm font-bold text-slate-300 uppercase tracking-widest mb-3" style={{ fontFamily: 'Manrope' }}>
              종목코드 입력
            </p>
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-12 pr-4 py-4 text-base focus:border-indigo-500 focus:outline-none transition-all text-white placeholder:text-slate-600"
              />
            </div>
            <button onClick={handleTicker}
              className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-4 rounded-xl text-base transition-all active:scale-95 shadow-lg shadow-indigo-500/20"
              style={{ fontFamily: 'Manrope' }}>
              조회
            </button>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-slate-600 text-sm">또는</span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>
          ...
          <div>
            ...
            <label className="block w-full border-2 border-dashed border-slate-700 rounded-xl p-8 text-center cursor-pointer hover:border-indigo-500 transition-colors group">
              <span className="material-symbols-outlined text-4xl text-slate-600 group-hover:text-indigo-400 transition-colors block mb-2">upload_file</span>
              <p className="text-base text-slate-400 font-medium mb-1">클릭하여 파일 업로드</p>
              <p className="text-sm text-slate-600">CSV · Excel · JSON</p>
          ...
          {uploadResult && (
            <div className="border border-slate-700 rounded-xl p-4 bg-slate-900/50">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">파싱 결과</p>
              <p className="text-base text-white font-mono mb-1">{uploadResult.row_count}행 · {uploadResult.data_type}</p>
              {uploadResult.ai_mapped_columns && Object.keys(uploadResult.ai_mapped_columns).length > 0 && (
                <p className="text-sm text-indigo-400">✨ AI 매핑 {Object.keys(uploadResult.ai_mapped_columns).length}개 컬럼</p>
              )}
          ...
          {companyName && (
            <div className="border border-slate-700 rounded-xl p-4 bg-slate-900/50">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">조회 종목</p>
              <p className="text-xl font-bold text-white font-mono">{companyName}</p>
              <p className="text-sm text-indigo-400 mt-1">KRX Live Data</p>
            </div>
          )}
        </div>
      ...
      {/* FAB */}
      <label className="fixed bottom-8 right-8 w-14 h-14 bg-indigo-500 rounded-full shadow-2xl shadow-indigo-500/40 flex items-center justify-center text-white hover:scale-110 active:scale-95 transition-all z-50 cursor-pointer">
