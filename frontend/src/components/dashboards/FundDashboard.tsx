...
export default function FundDashboard({ metrics, ohlcv, dartData }: Props) {
  ...
  const hasReturns =
    fmt.hasValue(returns) &&
    returns?.cumulative_return != null
  ...
  const hasRiskAdj =
    fmt.hasValue(riskAdj) &&
    riskAdj?.sharpe != null

  const hasFundFinancial =
  ...
  if (hasRiskAdj && riskAdj.sharpe < 0) {
    warnings.push(
      '샤프지수 음수 — 무위험자산 수익률 미달'
    )
  }

  if (hasRisk && Math.abs(risk.mdd) > 0.2) {
    warnings.push(
      `MDD ${fmt.pct(risk.mdd)} — 위험 관리 필요`
    )
  }

  if (
    credit?.current_ratio &&
    credit.current_ratio < 100
  ) {
    warnings.push(
      `유동비율 ${fmt.num(
        credit.current_ratio,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!hasReturns &&
        ...
        !hasFundFinancial && (
          <div className="bg-[#f5f2fe] border border-[#c7c4d7] rounded-xl p-4 text-center">
            <p className="text-sm text-[#767586]">
              수익률 계산에 필요한 주가(종가) 데이터가 없습니다.
            </p>

            <p className="text-xs text-[#c7c4d7] mt-1">
              종목코드 조회 또는 NAV/종가 포함 파일을 업로드해주세요.
            </p>
          </div>
        )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        ...
        {hasReturns && (
          <>
            <KpiCard
              label="누적 수익률"
              value={fmt.pct(returns.cumulative_return)}
              positive={returns.cumulative_return > 0}
            />
        ...
        {hasRiskAdj && (
          <>
            <KpiCard
              label="샤프지수"
              value={fmt.num(riskAdj.sharpe)}
              positive={riskAdj.sharpe > 1}
              sub="기준 1 이상"
            />

            <KpiCard
              label="연율화 수익률"
              value={fmt.pct(riskAdj.ann_return)}
              positive={riskAdj.ann_return > 0}
            />

            {riskAdj.calmar != null && (
              <KpiCard
                label="칼마 비율"
                value={fmt.num(riskAdj.calmar)}
                positive={riskAdj.calmar > 1}
                sub="기준 1 이상"
              />
            )}
          </>
        ...
        {hasRisk && (
          <>
            <KpiCard
              label="MDD"
              value={fmt.pct(risk.mdd)}
              color="#3B82F6"
            />

            <KpiCard
              label="연율화 변동성"
              value={fmt.pct(risk.volatility)}
            />
          </>
        )}
        ...
        {portfolioRisk && (
          <>
            {portfolioRisk.beta != null && (
              <KpiCard
                label="베타 (β)"
                value={fmt.num(portfolioRisk.beta)}
                positive={portfolioRisk.beta < 1}
                sub="기준 1 미만"
              />
            )}
      ...
      <div className="bg-white border border-[#c7c4d7] rounded-xl p-4">
        <p className="text-[#1b1b23] text-sm font-medium mb-3">
          컴플라이언스 모니터링
        </p>
      ...
      {hasFundFinancial && (
        <div className="flex flex-col gap-3">
          <p className="text-[#1b1b23] text-sm font-medium">
            재무 건전성 — 편입 적합성 판단
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            ...
            {valuation?.operating_margin != null && (
              <KpiCard
                ...
                value={
                  fmt.num(
                    valuation.operating_margin,
                    1
                  ) + '%'
                }
                positive={
                  valuation.operating_margin > 0
                }
                sub="수익성"
              />
            )}
        ...
        shareholders.length > 0 && (
          <div className="bg-white border border-[#c7c4d7] rounded-xl p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <p
                className="text-[#1b1b23] text-sm font-bold"
            ...
            <div className="flex flex-col divide-y divide-[#f0edf8]">
              ...
              {shareholders
                ...
                .map((sh: any, i: number) => (
                  ...
                  >
                    <div>
                      <span className="text-sm text-[#1b1b23] font-medium">
                        {sh.name}
                      </span>

                      {sh.relation && (
                        <span className="ml-2 text-xs text-[#767586]">
                          {sh.relation}
                        </span>
                      )}
                    </div>
                        ...
                        <div className="w-24 h-1.5 bg-[#f0edf8] rounded-full overflow-hidden">
                          ...
                          <div
                            ...
                            style={{
                              width: `${Math.min(
                                100,
                                parseFloat(sh.ratio) * 3
                              )}%`,
                            }}
                          />
                        </div>

                        <span className="text-sm font-bold text-[#4648d4] w-12 text-right">
                          {sh.ratio}%
                        </span>
                      </div>
                    )}
      ...
      {shares &&
        ...
        shares.items.length > 0 && (
          <div className="bg-white border border-[#c7c4d7] rounded-xl p-4">
            ...
            <p
              className="text-[#1b1b23] text-sm font-bold mb-3"
              style={{ fontFamily: 'Manrope' }}
            >
              주식 발행 현황

              <span className="text-[10px] text-[#767586] font-normal ml-1">
                {shares.year}년
              </span>
            </p>
            ...
            <div className="flex flex-col gap-3">
              ...
              {shares.items.map(
                (item: any, i: number) => (
                  ...
                  >
                    <span className="text-[#767586] font-medium">
                      {item.type}
                    </span>

                    <div>
                      <p className="text-[#767586]">
                        발행총수
                      </p>
